...
int writen(int fd, char *buf, int nbytes, struct sftpheader *header)
{
    ...
    {
    if((b=write(fd, buf, 1) )<=0) perror("write from socket: ");
    }
...
int readHeader(int fd, struct sftpheader *header)
{
   ...
   if(setMSG==1)
   {
	...
	short data_size; 
    	if (read(fd, (char *) &data_size, 1) != 1) return (-2);
    	if (read(fd, (char *) (&data_size)+1, 1) != 1) return (-2);
...
int log_client(char * ipv4addr, char *msg){
	...
	if((txtFile = fopen("LOG_FILE.txt", "a+")) < 0){
	return -1;
	}
	...
	fprintf(txtFile, "Client [ip: %s]: %s at %s\n", ipv4addr, msg, timebuf);
	fprintf(file, "Client [ip: %s]: %s at %s\n", ipv4addr, msg, timebuf);
	printf("Client [ip: %s]: %s at %s\n", ipv4addr, msg, timebuf);
